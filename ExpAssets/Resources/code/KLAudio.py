...
class Tone(AudioClip):
	...
	def __init__(self, duration, wave_type='sine', frequency=432, volume=1.0, fade_in=None, fade_out=None):
		...
		tone = self.__generate_tone(wave_type, frequency, fade_in, fade_out, duration)
		super(Tone, self).__init__(np.c_[tone, tone], volume)

	def __generate_tone(self, wavetype, hz, fade_in, fade_out, duration):
		...
		if wavetype == "sine":
			...
			if fade_in is not None:
				...
				# Adjust for fade_in, if requested
				n_fadein_samples = int(round(fade_in / 1000.0 * sample_rate))
				fade_in_arr = np.arange(n_fadein_samples) / float(n_fadein_samples)
				arr[:n_fadein_samples] = fade_in_arr * arr[:n_fadein_samples]

				# Adjust for fade_out, if requested
				n_fadeout_samples = int(round(fade_out / 1000.0 * sample_rate))
				fade_out_arr = np.arange(n_fadeout_samples) / float(n_fadeout_samples)
				fade_out_arr = fade_out_arr[::-1]  # Reverse fade_out array
				arr[-n_fadeout_samples:] = fade_out_arr * arr[-n_fadeout_samples:]

			# Bump everything back up to max
			arr = arr * max_int
